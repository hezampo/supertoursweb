...
class TrafficController extends I18nController {
    ...
    private $sql_all_tour = "(select tours.id, tours.code_conf, tours.id_client, clientes.firstname, clientes.lastname, tours.starting_date, tours.ending_date,
                               tours.adult, tours.child, 'MULTI' as type,  hoteles.nombre as nombre_hotel,  agencia.company_name as customer, agencia.type_rate, tours.length_day as days, tours.length_nights as nights,
                                                         tours.total as total, tours.tipo_pago as tipo_pago, transfer_in.type_transfer as transfer_in, transfer_out.type_transfer as transfer_out, tours.id_reserva
                             from tours
                               left join clientes
                            ...
                            where tours.starting_date<=? and tours.ending_date >=?)
                            union
                            ...
                            tours_oneday.total as total, reservas.tipo_pago as tipo_pago, '' as transfer_in, '' as transfer_out, tours_oneday.id_reserva
                            from tours_oneday
                            left join clientes
                            ...
                            where tours_oneday.starting_date<=? and tours_oneday.ending_date>=?)
                            union
                            ...
                            reservas.totaltotal as total, reservas.tipo_pago as tipo_pago, '' as transfer_in, '' as transfer_out, reservas.id as id_reserva
                            from reservas
                            left join clientes
                            on clientes.id=reservas.id_clientes
                            left join agencia
                            on agencia.id=reservas.agency
                            where (reservas.fecha_salida=? or reservas.fecha_retorno=?) and (reservas.type_tour = ''));";
    ...
    function generate_and_search_traffics($id_tour, $type_tour, $solo_parques=false){
        $tour = null;
        if($type_tour == 'MULTI'){
            $tour = new Tours();
        }elseif($type_tour == 'ONE'){
            $tour = new Tour_oneday();
        }elseif($type_tour == 'RESERVE'){
            $tour = new Reserve();
        }
  ...
  left join driver
    ...
    public function daily_arrival_pdf(){
        ...
        foreach($tours as $tour):
            ...
            $page .= '<td style="width:40px;" >'.$tour['firstname'].' '.$tour['lastname'].'</td>';
            ...
            if($tour['id_reserva'] != -1){
                ...
                if(isset($tour['transfer_in'])){
                    $in = $tour['transfer_in'];
                }
                if(isset($tour['transfer_out'])){
            ...
            $collect = 0;
            if($tour['tipo_pago'] == 'COLLECT ON BOARD'){
                $collect = $tour['total'];
            ...
            $page .= '<td rowspan="2" style="width:130px; font-size:10px;" >'.trim($in).' '.$rate.' - '.$tour['type'].' '.$tour['days'].$tour['nights'].' '.strtoupper($tour['nombre_hotel']).'</td>';
            $page .= '<td rowspan="2" style="width:100px; font-size:10px;">'.trim($tour['customer']).'</td>';
            $page .= '<td rowspan="2" style="width:50px; font-size:10px;">'.trim($out).'</td>';
            ...
            $page .= '<td rowspan="2" style="width:20px; font-size:10px;" align="center">'.number_format($collect, 2, '.', '.').'</td>';
            ...
            for($i=0; $i<7 ; $i++){
                ...
                $page .= '<td style="width:30px; height:15px; font-size:8px;">'.$name_service.'</td>';
            }

            $page .= '</tr>';
            $page .= '<tr class="row'.($cont%2).'" ><td><span style="font-size: 8px; padding-left: 5px;">'.$tour['code_conf'].'</span></td>';
            for($i=7; $i<14 ; $i++){
                $name_service = '';
                if($i < $cant_services){
                    $service = $services[$i];
...
//
        ...
        $page .= '<tr class="trInicial">
          ...
          <th >'.number_format($total_collect, 2, '.', '.').'</th>
          <th colspan="7">&nbsp;</th>
        </tr>';
    ...
    function guides_services_pdf(){
        ...
        foreach($traffics as $traffic){
            ...
            $page .= '<tr class="row0" >';
            $page .= '<td style="width:170px;" >'.$traffic['firstname'].' '.$traffic['lastname'].'</td>';
