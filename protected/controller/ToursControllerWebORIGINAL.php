...
//                    exit;
                ...
                if ($id == 0 && $id2 == 2 && isset($fecha)) {
                    ...
                    echo '<br><br><div align="center" style="margin-top: -9px;">
                                                        <table class="table table-hover">
                                                            <thead>
                                                                 <tr class="warning">
                                                                         <th width="10%">Select</th>
                    ...
                    if (count($salida) > 0 && $tiempo) {
                        ...
                        foreach ($salida as $e) {
                            ...
                            if ($disponible > 0) {
                                ...
                                echo '<tr class="nodisponible" >';
                                echo '<td><img src="' . Doo::conf()->APP_URL . 'global/images/detalles.png" width="24px" height= "20px" title="The Trip '.$e['trip_no'].' does not have positions available"</td>';
                            }
                            echo    '<td>'.$e['trip_no'].'</td>
                                        <td nowrap="nowrap"  >' . date("g:i A", strtotime($e['trip_departure'])) . '</td>
                                        <td nowrap="nowrap"  >' . date("g:i A", strtotime($e ['trip_arrival'])) . '</td>
					...
					var id = 1;
		         	 ...
		         	 $("#pickups2").load("' . $url . 'tours/question14/" + id + "/" + trip_no2);});</script>';
                }
    ...
    public function question2() {
        try {
            ...
            $hotel = "hotel";
            if (empty($hoteles)) {
                echo '<div class="alert alert-danger" id="tablah">
                        <strong>Danger!</strong> No hotels available in this category.
                    </div>';
            } else {
                foreach ($hoteles as $datos) {
                    ...
                    if (!empty($array_imagenes)) {
                        ...
                        foreach ($array_imagenes as $values_img) {
                            if ($contador_img == 1) {
                                $html_img .= '<a class="galeria" title="' . $values_img->descripcion . '" data-lightbox-gallery="' . $id_hotel . '" href="' . Doo::conf()->APP_URL . 'global/' . $values_img->ruta_resize . '"> <img class="img img-thumbnail" src="' . Doo::conf()->APP_URL . $datos ['image1'] . '"/> </a>';
                            } else {
                        ...
                        $html_img .= '<img class="img img-thumbnail" src="' . Doo::conf()->APP_URL . $datos ['image1'] . '"/>';
                    }



                    /* aqui comienza el panel de reserva para elegir el hotel */
                    echo '<div class="panel panel-borde1 col-sm-12" style="background-color: #eee;">
                                <div class="col-sm-4">
                                '.$html_img.'
                                </div>
                                <div class="col-sm-8 panel-mediano">
                                    <h1 style="text-align: -webkit-left; color: #000;"> <i class="fa fa-building" style="color: brown;"></i>'.$datos ['nombre'].' <br />
                                    '.($datos ["categoria"] == 3 ? '<img src="' . Doo::conf()->APP_URL . 'global/img/3.png"  border="0" />' : '' ).($datos ["categoria"] == 2 ? '<img src="' . Doo::conf()->APP_URL . 'global/img/2.png"  border="0" />' : '' ) . ($datos ["categoria"] == 4 ? '<img src="' . Doo::conf()->APP_URL . 'global/img/4.png"  border="0" />' : '' ) . '
                                    </h1>
                                    <div align="center" style="text-align:justify;font-size: 16px;" >'.$datos ['description'].'</div>
                                    <font color="#006633;margin-left: -79%;"> '.($datos ["breakfast"] == 1 ? '<i class="fa fa-coffee" style="color: #006633;"></i> Free Breakfast' : '').'</font>
                                    
                              
                       <div id="choice" align="right" style="margin: 0 0 13px;"   >
						';
                    ...
                    echo '</div> </div></div>';
                    /* aca termina el panel de reserva para el hotel */
            ...
            try {
                ...
                if (!empty($mgk)) {
                    ...
                    foreach ($mgk as $values_img) {
                        if ($contador_img == 1) {
                            $html_img .= '<a class="galeria" title="' . $values_img->descripcion . '" data-lightbox-gallery="' . $id_parque . '" href="' . Doo::conf()->APP_URL . 'global/' . $values_img->ruta_resize . '"> <img class="img img-responsive"  src="' . Doo::conf()->APP_URL . 'global/img/Walt-Disney-World-HD-1.jpg"/> </a>';
                        } else {
                    ...
                    $html_img .= '<img class="img img-responsive"  src="' . Doo::conf()->APP_URL . 'global/img/Walt-Disney-World-HD-1.jpg"/>';
                }
...
//        $super_total1 = $_POST['super_total1'];
//        echo $super_total1;
        ...
        $_SESSION['onedaytour'] = false;
        if (!isset($_SESSION ['user'])) {//&& isset($cellular_tick) 
            if (!(isset($firstname_tick) && isset($lastname_tick) && isset($email_tick) && isset($phone_tick) && isset($_SESSION['toursbooking']) )) {
                if (isset($_SESSION['data_agency'])) {
        ...
        if (!isset($_SESSION ['user'])) {//&& isset($cellular_tick) 
            ...
            $_SESSION ["tourstick"] = $tourstick;
            $this->data['rootUrl'] = Doo::conf()->APP_URL;
            $this->renderc('tours/loginuser', $this->data);//este no se puede borrar por nada del mundo
        } else {
            $login = $user = $_SESSION ['user'];
            if (isset($_SESSION['data_agency'])) {
                $tourstick = array(
...
//                WHERE t1.type_rate = 0  AND t1.trip_no = ?";
  ...
  <tr>
    <td width='20%' style='color: #fff;'>Areas <i class='glyphicon glyphicon-map-marker'></i>:</td>
    <td width='20%'>";
  ...
  <tr>
    <td width='30%' style='color: #fff;'> Pickup Point <i class='glyphicon glyphicon-map-marker'></i>:</td>
    <td>";
            echo "<select class='form-control' name='pickup' id='pickup' >";
  ...
  <!--tr>
    ...
    <td>";
            ...
            // gropoff orlando
            echo "<select name='pickup' id='pickup'>";
            ...
            echo "<div id='regreso' style='margin-top: -3%;'><a href='#' id='back' class='btn btn-primary'><i class='fa fa-reply-all'></i> Back to trip</a></div>";
...
$("#back").click(function() {
  ...
  <tr>
    ...
    <td width='20%'>";
            ...
            echo "<select class='form-control'  name='area2' id='area2'>";
            foreach ($areas as $e) {
                if ($idA1 == $e ['trip_to']) {
                    $select = 'selected';
                } else {
  ...
  <!--tr>
    ...
    <td>";
            echo "<select name='pickup2' id='pickup' >";
            foreach ($pikuporlando as $e) {
                echo '<option value="' . $e ['id'] . '"  >' . $e ['place'] . '  ' . $e ['address'] . '</option>';
            }
  ...
  <tr>
    <td width: '40%' style='color: #fff'>Pickup Point <i class='glyphicon glyphicon-map-marker'></i></td>
    <td>";

            // gropoff area escogida
            echo "<select class='form-control'  name='pickup2' id='pickup2'>";
            foreach ($pickupdropof as $e) {
                echo '<option value="' . $e ['id'] . '"  >' . $e ['place'] . '  ' . $e ['address'] . '</option>';
            }
            echo "</select>";
            ...
            echo "<div id='regreso' style='margin-top: 2%;'><a href='#' id='back2' class='btn btn-primary'> <i class='fa fa-reply-all'></i> Back to trip</a></div>";
...
$("#back2").click(function() {
  ...
  return false;
    ...
    public function exten() {
        ...
        $pickupdropof = $rs->fetchAll();
        echo "<select name='pickup" . $num . "' id='pickup" . $num . "' style=''>";
        foreach ($pickupdropof as $e) {
            echo '<option value="' . $e ['id'] . '"  >' . $e ['place'] . '  ' . $e ['address'] . '</option>';
        }
        echo "</select>";
    ...
    public function onedaytour_two2() {
        ...
        if (isset($toursbooking ['trip1'])) {
            if ($dat->type_rate == 1) {// Se busca SpecialNet
                ...
                $sql = 'SELECT onet.priceadult_WDW, onet.pricechild_WDW	FROM  onetour onet WHERE onet.type_rate = ? AND annio = ?';
                $type = 0;
                $rs = Doo::db()->query($sql, array($dat->type_rate, $fecha));
                $prices = $rs->fetch();
            }
...
WHERE id_area = ? AND type_web = ? ORDER BY id ASC";
  ...
  <!--tr>
    ...
    <td>";
            ...
            // gropoff orlando
            echo "<select name='pickup' id='pickup' style=''>";
            foreach ($pikuporlando as $e) {
...
$("#area").change(function() {			
  ...
  <tr>
    ...
    <td>";
            ...
            // gropoff area escogida
            echo "<select name='pickup2' id='pickup2' >";
            foreach ($pickupdropof as $e) {
                echo '<option value="' . $e ['id'] . '"  >' . $e ['place'] . '  ' . $e ['address'] . '</option>';
            }
        ...
        try {
            ...
            if (isset($this->params ["ides"])) {
                ...
                if ($parkdatos ['adult'] && $parkdatos ['child']) {
                    ...
                    /////////////////////////////////services
                    if (isset($toursbooking ['trip1'])) {
                        if ($dat->type_rate == 0) {// Se busca SpecialNet
                            $sql = 'SELECT onet.priceadult,
                                    onet.pricechild,
                                    onet.suplemag_adult,
                                    onet.suplemag_child,
                    ...
                    if ($especial) {
                        ...
                        //echo '<pre>';
                        //print_r($array_parks_asoc_price_adult);
                        //echo '</pre>';
    ...
    public function tours_tow_onedaytour() {
        try {
            ...
            list($idPark,$idGrupo) = explode(",", $ides);
            $id_park = $idPark;
            $id_grupo = $idGrupo;
            ...
            $dat->id = -1;
            $divporpax = 0;
            //$_SESSION ['toursbooking'] ['np'];
            
            if ($_SESSION ['toursbooking']['np'] == 0 && $_SESSION ['toursbooking'] ['parktotal'] == 1) {
                $_SESSION['toursbooking']['ticketpark'] = 0;
                $divporpax = 0;
            } else {
                //echo 'entra en esta parte de la condicion';
                $grupos = $_SESSION ['grupos'];
                $toursbooking = $_SESSION ['toursbooking'];
                $clave = $_SESSION ['grupos']['id'] = $id_park; 
                $park = $_SESSION ['grupos']['idParque'] = $id_grupo;
                
                $sql = 'SELECT id,adults1,child1,id_grupo,id_parque,cantidad
				FROM admin_parques_tarifa
				WHERE type_rate = "1" 
                                AND id_agency = "-1" 
                                AND id_parque = ? 
                ...
                $sql0 = 'SELECT id,adults1,child1,id_grupo,id_parque,cantidad,type_rate, id_agency
				FROM admin_parques_tarifa
				WHERE type_rate = 2 
                                AND id_parque = ? 
                                AND  id_grupo = ? 
                ...
                if ($dat->id != -1) {
                    $rs = Doo::db()->query($sql0, array(trim($park), trim($clave), $dat->id, $fecha));
                    $consulta = $rs->fetch();
                    $val = $rs->rowCount();
                    if (empty($consulta)) {
                        $rs = Doo::db()->query($sql, array(trim($park), trim($clave), $fecha));
                        $consulta = $rs->fetch();
                        $val = $rs->rowCount();
                    }
                } else {
                    //echo 'entra aca';
                    $rs = Doo::db()->query($sql,array(trim($id_park), trim($id_grupo), $fecha));
                    $consulta = $rs->fetchAll();
                    $val = $rs->rowCount();
                }
                if ($val > 0) {
                    #'actualmente estoy aqui';
                    foreach ($consulta as $c) {
                        $precioadulto = $c['adults'];
                        $precionino = $c['child'];
                        $id_grupo = $c['id_grupo'];
                        $id_parque = $c['id_parque'];
                    }
                    $priceadult = $toursbooking ['adults'] * $precioadulto;
                    $priceachild = $toursbooking ['childs'] * $precionino;
                    ...
                    $_SESSION['id_parque'] = $id_parque;
                } else { 
                    
                    $id_parque1 = $_SESSION['id_parque'];
                    //echo $id_parque1;
                    $id_grupo1 = $_SESSION['id_grupo'];
                    //echo $id_grupo1;
                    
                    $clave = $toursbooking['grupos']['id_grupo'];
                    $park = $toursbooking['grupos']['id_parque'];
                    
                    $fecha_ini = strtotime($toursbooking["fecha_llegada"]);
                    ...
                    if ($dat->id != -1) {
                        ...
                        $val = $rs->rowCount();
                        if (empty($consulta)) {
                            $rs = Doo::db()->query($sql, array(trim($park), trim($clave),$fecha,$fecha_ini));
                            $consulta = $rs->fetch();
                            $val = $rs->rowCount();
                        }
                    } else {
                        
                        /*En esta parte del controlador podemos imprimir 
                        el resultado de la busqueda ya que el dat->id 
                        es igual a -1.*/
                        
                        $GLOBALS['id_grupo'];
                        $rs = Doo::db()->query($sql, array($id_parque1,$id_grupo1,$fecha,$fecha_ini,$fecha_ini));
                        $consulta = $rs->fetch();
                        echo '<pre>';
                            print_r($consulta);
                        echo '</pre>';
            ...
            if (!isset($_SESSION ['user'])) {
                ...
                if ($pago < "3") {
                    ...
                    try {
                        ...
                        $mail->AddAddress("henry@supertpurs.com", "");
                        $mail->MsgHTML($contenido);
                        $mail->Send();
                    } catch (phpmailerException $e) {
                        echo $e->errorMessage(); // Errores de PhpMailer
...
//                            }
    ...
    function quotationOnedayTour() {
        try {
            extract($_POST, EXTR_SKIP);//&& isset($phone_tick)
            if (!(isset($firstname_tick) && isset($lastname_tick) && isset($email_tick) && isset($cellular_tick)  && isset($_SESSION['toursbooking']))) {
                if (isset($_SESSION['data_agency'])) {
                    return Doo::conf()->APP_URL . "agency/#tours2";
                } else {
                    return Doo::conf()->APP_URL . "one-day-tour/";
