...
//                    $login->address = $signup->address;
    ...
    public function load() {//esta función es la que guarda en la bd después del pago
        ...
        $clientef = '{"ssl_first_name":"'.$ssl_first_name.'","ssl_last_name":"'.$ssl_last_name.'","ssl_phone":"'.$ssl_phone.'","ssl_country":"'.$ssl_country.'","ssl_state":"'.$ssl_state.'","ssl_city":"'.$ssl_city.'","ssl_zip_code":"'.$ssl_zip_code.'","ssl_passengers_name":"'.$ssl_passengers_name.'","ssl_email":"'.$ssl_email.'","ssl_avs_address":"'.$ssl_address.'"}';
...
//                    $fs->id = $fs->insert();
                ...
                $insert = 'INSERT INTO reservas_agency (id, id_reservas, id_agencia, id_cliente, type_client, id_useragency, paid_type, metodo_paid, paid_net, paid_full, otheramount, agency_fee, comision, paper_voucher)
                        VALUES ("", "' . $reserves->id . '", "53", "' . $login->id . '", "1", "963", "PRED-PAID", "CREDIT CARD WITH FEE-FULL", "' . $reserves->total2 . '", "' . ($reserves->total2 + $_SESSION['booking']['fee']) . '", "0", "0", "0", "0")';
                Doo::db()->query($insert);
           ...
           // print_r($chargeJson);
            ...
            if($chargeJson['amount_refunded'] == 0 && empty($chargeJson['failure_code']) && $chargeJson['paid'] == 1 && $chargeJson['captured'] == 1){ 
                ...
                if($charge->status=="succeeded"){
                        ...
                        $clienteJson = '{"ssl_first_name":"'.$ssl_first_name.'","ssl_last_name":"'.$ssl_last_name.'","ssl_phone":"'.$phone.'","ssl_country":"'.$country.'","ssl_state":"'.$state.'","ssl_city":"'.$city.'","ssl_zip_code":"'.$zip_code.'","ssl_passengers_name":"'.$passengers_name.'","ssl_email":"'.$email.'","ssl_avs_address":"'.$address.'"}';
