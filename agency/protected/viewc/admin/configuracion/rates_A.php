<link rel="stylesheet" type="text/css" href="<?php echo $data['rootUrl']; ?>global/themes/blitzer/jquery-ui-1.9.0.custom.css" />

<link rel="stylesheet" type="text/css" href="<?php echo $data['rootUrl']; ?>global/themes/blitzer/jquery-ui-1.9.0.custom.min.css" />

<script type="text/javascript" src="<?php echo $data['rootUrl']; ?>global/js/jquery-ui-1.8.18.custom.min.js"></script>





<?php if(isset($_REQUEST['msg'])){?>

    <div class="error" style="margin-top: 10px;"><?php /*echo $_REQUEST['msg'];*/ ?></div>

<?php } ?>



<form name="form1" method="post" action="<?php echo $data['rootUrl']; ?>admin/tours/room-rates"  class="form" id="form1">

    <div id="header_page" >

        <div class="header"><STRONG>Rates A</STRONG></div>

        <div  id="toolbar">



            <div class="toolbar-list">

                <ul>



                    <li class="btn-toolbar">

                        <a href="<?php echo $data['rootUrl']; ?>admin/tours/room-rates/add" id="btn-add" class="link-button">

                            <span class="icon-new" title="Nuevo" >&nbsp;</span>

                            New

                        </a>

                    </li>



                    <li class="btn-toolbar">

                        <a href="<?php echo $data['rootUrl']; ?>admin/tours/room-rates/edit" id="btn-edit" class="link-button" >

                            <span class="icon-edit" title="Editar" >&nbsp;</span>

                            Edit

                        </a>

                    </li>



                    <li class="btn-toolbar">

                        <a href="<?php echo $data['rootUrl']; ?>admin/tours/room-rates/delete" id="btn-delete" class="link-button">

                            <span class="icon-delete" title="Eliminar" >&nbsp;</span>

                            Delete

                        </a>

                    </li>



                    <li class="divider">&nbsp;</li>



                    <li class="btn-toolbar">

                        <a href="<?php echo $data['rootUrl']; ?>admin/home" id="btn-back" class="link-button">

                            <span class="icon-back" title="Regresar" >&nbsp;</span>

                            Back

                        </a>

                    </li>



                </ul>

                <div class="clear"></div>

            </div>

            <div class="clear"></div>

        </div>

    </div>

    </div>

    <div id="form" >



        <div id="filter-bar" >



            <label style="width:70px" class="filter-by">Filter By</label>

            <select name="filtro" id="filtro" class="select">

                <option value="codigo" <? echo $data["filtro"] == 'codigo' ? 'selected' : '' ?>>Hotel Code</option>

                <option value="nombre" <? echo $data["filtro"] == 'nombre' ? 'selected' : '' ?>>Hotel Name</option>

            </select>



            <span class="search">

                <input name="texto" type="text" size="30" maxlength="30" class="input-search" id="texto" value="<?php echo $data["texto"] ?>"/>

                <input type="button" class="search-btn" id="btn-find" />

            </span>



        </div>



        <div id="datagrid">

            <table class="grid"  align="center" cellspacing="1"  id="grid">

                <thead>

                <tr>

                    <th width="20">&nbsp;</th>

                    <th width="90">Hotel Name</th>

                    <th width="60">Starting Date</th>

                    <th width="55">Ending Date</th>

                    <th width="40">Sgl Price</th>

                    <th width="40">Dbl Price</th>

                    <th width="40">Tpl Price</th>

                    <th width="40">Qua Price</th>
                    
                    <th width="40">Sgl2 Price</th>

                    <th width="40">Dbl2 Price</th>

                    <th width="40">Tpl2 Price</th>

                    <th width="40">Qua2 Price</th>

                   <!-- <th width="40">Breakfast</th>

                    <th width="40">Resort </th>-->

                </tr>

                </thead>

                <tbody>

                <?

                $i = 0;

                foreach ($data["ratesroom"] as $e):

                    ?>

                    <tr class="row<? echo $i ?>">

                        <td><input name="item" type="radio" value="<? echo $e['id']; ?>" /></td>

                        <td><? echo $e['nombre']; ?></td>

                        <td><? echo date("m-d-Y",$e['fecha_ini']); ?></td>

                        <td><? echo date("m-d-Y",$e['fecha_fin']); ?></td>

                        <td><? echo $e['sgl']; ?></td>

                        <td><? echo $e['dbl']; ?></td>

                        <td><? echo $e['tpl']; ?></td>

                        <td><? echo $e['qua']; ?></td>

                        <td><? echo $e['sgl2']; ?></td>

                        <td><? echo $e['dbl2']; ?></td>

                        <td><? echo $e['tpl2']; ?></td>

                        <td><? echo $e['qua2']; ?></td>
                        
                        <!--<td><? echo $e['brackfast']; ?></td>

                        <td><? echo $e['resortprice']; ?></td>-->

                    </tr>

                    <?php

                    $i = 1 - $i;

                endforeach;

                ?>

                </tbody>

                <thead><tr></tr>

                </thead>

            </table>

            <div id="pagination">

                <?php echo $data['pager'] ?>

            </div>

        </div>



</form>



</div>



<script type="text/javascript">

    $('#grid tr').click(function() {

        $(this).find('input[name="item"]').prop('checked', true)



    });

    $('texto').keypress(function(e){

        if (e.keyCode==13)

            $('#form1').submit();

    })



    $('#btn-find').click(function(){

        $('#form1').submit();

    });



    $('#btn-edit').click(function(e){

        var id = $('input[name=item]:checked').attr('value');

        if (!id){

            alert('You must select an Item');

            e.preventDefault();

        }

        else {

            var action = $(this).attr("href") + "/" + id;

            $(this).attr("href",action);

        }

    });



    $('#btn-delete').click(function(e){

        n = $('input[name=item]:checked').attr('value');

        if (!n){

            alert('You must select an Item');

            e.preventDefault();

        }else {

            if (confirm("Are you sure of deleting this item? ...")){

                var action = $(this).attr("href") + "/?item=" + n;

                $(this).attr("href",action);

            }

        }

    });



</script>







