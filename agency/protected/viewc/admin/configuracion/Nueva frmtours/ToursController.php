...
//        exit;
    ...
    public function loadDatos() {
        ...
        if ($id == 'cliente') {
            ...
            $sql = "SELECT  id,company_name,tour_name FROM agencia WHERE company_name LIKE ?  LIMIT 5  ";
            ...
            if (count($dataList) >= 1) {
              ...
              echo "<script>
                        document.getElementById('type_rate').value= '0';
            ...
            $sql = "SELECT  id,firstname, lastname FROM user_agencia WHERE firstname LIKE ? and id_agencia = ?  LIMIT 5  ";
...
//        echo'</pre>';
//        exit();
         //$this->renderc('admin/frm_tours', $this->data);
...
//                    echo $type_rate;
            ...
            $sql = "select 
                         ...
                         left join areas  t4 on (t2.trip_from = t4.id)
                         left join areas  t5 on  (t2.trip_to  = t5.id)
                         where t2.type_rate = " . $dat->type_rate . " and t2.trip_from = ? AND t2.trip_to = ? and t1.fecha = ? AND t2.trip_departure > '' and t1.estado = '1' and t2.anno = ? and t2.fecha_ini = '$fecha_ini1'   ORDER BY t2.trip_departure ASC";
...
//                    $tarifa1 = $type_rate;
            ...
            $sql = "select 
                         ...
                         left join areas  t4 on (t2.trip_from = t4.id)
                         left join areas  t5 on  (t2.trip_to  = t5.id)
                         where t2.type_rate = " . $dat->type_rate . " and t2.trip_from = ? AND t2.trip_to = ? and t1.fecha = ? AND t2.trip_departure > '' and t1.estado = '1' and t2.anno = ? and t2.fecha_ini = '$fecha_sal1' ORDER BY t2.trip_departure ASC";
...
//        exit;
    ...
    public function totalValorPark($adult, $child, $id_agency = -1) {
        ...
        $sql = 'SELECT adults,child,id_parque ,cantidad
		FROM admin_parques_tarifa
		WHERE type_rate = 1 AND  id_agency = -1 AND id_grupo = ? AND  cantidad = ?  AND fecha_fin >= ? AND year = '.$yearact.'';
...
//                            exit();
                            ...
                            $fecha3 = strtotime($fecha2);
                            $fecha_ini = $fecha3;
                            //echo '<br />dentro del if 1= '.$fecha3;
                            
                            $fecha_salida1 = $this->params["fecha_salida"];
                            $fecha11 = $fecha_salida1;
                            list ($dia1, $mes1, $anyo1) = explode("-", $fecha11);
                            $fecha21 = $mes1 . "-" . $dia1 . "-" . $anyo1;
                            $fecha31= strtotime($fecha21);
                            $fecha_ini11 = $fecha31;
                            //echo '<br />dentro del if 2= '.$fecha31;
...
//                            $fechaPrueba = strtotime($fecha2);
//                            echo 'dentro del if '.$fechaPrueba;
