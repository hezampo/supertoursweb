...
class OnedaytourController extends I18nController {
    ...
    public function index() {
        ...
        if (isset($this->params['pindex']))
            ...
            $pager->paginate(1);
        $sql = "SELECT id, priceadult_WDW, pricechild_WDW, type_rate, id_agency, annio
    ...
    public function getPriceParks() {
        ...
        $sql = "SELECT p.nombre, t.adults, t.child, t.cantidad, p.id_grupo AS gid, g.nombre AS gnombre
            ...
            LIMIT 1";
        $rs = Doo::db()->query($sql, array($id_park, $id_park, $id_agency, $fecha));
...
//                         $("#total_sumplemento").val(' . $suplemento_suma . ');    
    ...
    public function getCosto() {
        ...
        if ($dat->type_rate == 1) {// Se busca SpecialNet
            ...
            if (empty($prices)) {//Si no encuentra Buscamos Net.
                $sql = 'SELECT onet.priceadult, onet.pricechild,onet.pricechild	FROM  onetour onet 
...
//            unset($_SESSION['codconf']);
        ...
        try {
            ...
            $sql = "SELECT p.nombre, t.adults, t.child, t.cantidad, p.id_grupo AS gid, g.nombre AS gnombre
                    ...
                    ORDER BY t.cantidad DESC 
                    LIMIT 1";
            $rs = Doo::db()->query($sql, array($id_selected_park, $id_selected_park, $id_agency, $fecha));
            $parks_rates_rows = $rs->rowCount();
            if ($parks_rates_rows == 0) {
                $sql = "SELECT p.nombre, t.adults, t.child, t.cantidad, p.id_grupo AS gid, g.nombre AS gnombre
                    FROM parques AS p, admin_parques_tarifa AS t, grupo_parques AS g
...
//            $reserve->precioN = $pricexchild * $child;
            ...
            $reserve->precio_trip1_a = $prices["priceadult"];
            $reserve->precio_trip1_c = $prices["pricechild"];
            $reserve->precio_trip2_a = $prices["priceadult"];
            $reserve->precio_trip2_c = $prices["pricechild"];

            $reserve->precioA = $prices["priceadult"]; //--
            if ($child > 0) {
                $reserve->precioN = $prices["pricechild"]; //--
            }
            ...
            /** Discriminacion de precios */
            $tour->t_price_adult = $prices["priceadult"];
            $tour->t_price_child = $prices["pricechild"];
            if ($transport > 0) {
            ...
            if ($opcion_pago == '3') {//"Credit Card+ 4 % FEE
                if ($otheramount > 0) {
                    $total_total_new_outa = $total_total_new + ($otheramount * 0.03);
                } else {
                    $total_total_new_outa = $total_total_new + ($total_total_new * 0.03);
                }

                $tres_porciento = ($otheramount * 0.03);
            } else {
                $tres_porciento = 0;
            }
            if ($extra > 0) {
            ...
            $rastrostring .= "Total Cost = " . $total_total_new . '&';
...
//            return Doo::conf()->APP_URL . "admin/onedaytour";
            ...
            if ($opcion_pago == 3 || $opcion_pago == 4 || $opcion_pago == 6) {

                if ($pay_amount != "" && $pay_amount > 0) {
                    $this->registrar_pago($tour, NULL, $login, $pay_amount, $prefix, "COLLECT ON BOARD");
                }
            }
    ...
    public function save_edited_oneday() {
        ...
        try {
            ...
            if ($reserve->fecha_ini != $y . '-' . $m . '-' . $d) {
                $reserve->fecha_ini = $y . '-' . $m . '-' . $d;
                $reserve->fecha_salida = $reserve->fecha_retorno = $y2 . '-' . $m2 . '-' . $d2;
                $reserve->tipo_ticket = "roundtrip";
                //cambio dia del tour
  ...
  <tr>";
    ...
    public function code_generation() {
        do {
            ...
            $codigo = $prefix . $mes . $y . $dia . rand(0, 9999);
