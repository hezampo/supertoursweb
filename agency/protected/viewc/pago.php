...
<!--  <form action="https://www.myvirtualmerchant.com/VirtualMerchant/process.do" method="POST">-->
 ...
 <!-- //echo  isset($_SESSION['agency_fee'])? ($booking["totaltotal"]+$_SESSION['agency_fee']):$booking["totaltotal"]; -->
 
 <?php 

	 $pago = $_SESSION['booking']['pago_pred'];
	 //$pago = 1;
...
<input type="hidden" name="ssl_city" value="<? echo $login->city; ?>">
<input type="hidden" name="ssl_state" value="<? echo $login->state; ?>">
<input type="hidden" name="ssl_description" value="Paid Online">
...
<input type="hidden" name="ssl_receipt_decl_get_url" value="<?php echo $data['rootUrl']; ?>transaction/decline">
<input type="hidden" name="ssl_receipt_apprvl_method" value="REDG">
<input type="hidden" name="ssl_receipt_apprvl_get_url" value="<?php echo $data['rootUrl']; ?>transaction/approval">
...
<br/>
  ...
  <tr>
   ...
   <tr>
	  <td>&nbsp;</td>
  	<td colspan="3" style="font-size:14px; font-family:Verdana, Geneva, sans-serif; color:#F00;"><b>Remember to charge only once and wait for the system to send the ticket confirmation to your email.</b></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  ...
  <tr>
    ...
    <td width="72%">Millions of passengers proved our red carpet service specially created to provide peace and comfort in traveling. Our purpose is to serve you like a king. <b> Copyright &copy;  1989 - 2012 Super Tours. All Rights Reserved.</b> </td>
    <td width="1%">&nbsp;</td>
  </tr>
</table>
</div>
   ...
   $("#btn-continue").click(function(event){
     ...
     if (!d1){
      ...
      if(total != $('input:[name=ssl_amount]').val()){
	   	...
	   	document.location.href = '<?php echo $data['rootUrl']; ?>transaction/decline';
	     return false;
		  }
