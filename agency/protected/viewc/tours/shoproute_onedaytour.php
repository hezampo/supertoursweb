<?php // $dat = $data['dat']; $acountAgen = new Agency_Account($_SESSION['agencyAcount']);?>
                                ...
                                <div id="contendinfo">
                                    <div id="mapmarcohome">
                                        ...
                                        <div id="contenidohometours"><br />
                                                ...
                                                <div id="content">
                                                <form id="form1" class="form" action="<?php echo $data['rootUrl']; ?>tours/userlog2" onsubmit="valid()" method="post" name="form1">
     <div id="byti">
     <div id="byti2">
                                                        ...
                                                        <tr valign="top">
                                                            <td width="100%"><table width="100%">
                                                              <tr valign="top">
                                                                ...
                                                                <td width="258"><font class="bkfont"><?php echo date("l", strtotime($toursbooking['fecha_llegada'])); ?>,<?php echo date("M", strtotime($toursbooking['fecha_llegada'])); ?> <?php echo date("d", strtotime($toursbooking['fecha_llegada'])); ?>,<?php echo date("Y", strtotime($toursbooking['fecha_llegada'])); ?></font></td>
                                                              </tr>
                                                              <tr valign="top">
                                                                <td>Arriving:</td>
...
LOCAL TRANSFERS TO PARKS</td>
  ...
  <tr>
    ...
    <table id="tableorder"  width="96%">
      <tr>
        <td height="28" colspan="6"  ><div align="right" ><strong><?php echo $toursbooking['adults'];?></strong> ADULTS AND <strong><?php echo $toursbooking['childs'];?></strong> CHILDREN TOTAL OF <strong><?php echo $toursbooking['totalpax'];?></strong> PASSENGERS </div></td>
        </tr>
    ...
    <div id="idIt">
   <?php include_once('ResponseItinerary_Onedaytour.php'); ?>
     </div>
       <br/> 
         <tr>
    <td height="20" colspan="2" id="titletd" ><strong>PRICE</strong></td>
  </tr>
    <tr><td>
       	<table style=" " width="90%" border="0" cellpadding="3" id="tableorder">
      <tbody><tr>
       ...
       <tr>
        <td height="27"></td>
        <td>&nbsp;</td>
        <td id="titlell"> Per Adults </td>
        <td width="20%" id="titlelp">$ <?php echo number_format($data["total_por_adultos"],2,'.',',');?></td>
      </tr>
	  <tr>
        <td height="27"></td>
        <td>&nbsp;</td>
        <td id="titlell"> Per Childs </td>
        <td width="20%" id="titlelp">$ <?php echo number_format($data["total_por_childs"],2,'.',',');?></td>
      </tr>
       <tr>
        <td height="27"></td>
        <td>&nbsp;</td>
...
color: #CE0000;
  ...
  <tr>
    <td height="20" colspan="2" id="titletd" ><strong>PAYMENT</strong></td>
  </tr>
  <td colspan="2" >
 ...
 <?php }  ?>   
      ...
      <tr>
        <td height="32" align="center"><strong>TOTAL AMOUNT for THIS TOUR:</strong> <STRONG id="total_amount" STYLE="COLOR:rED;" >$<? echo number_format($toursbooking['tqp'] * $toursbooking['totalpax'], 2, '.', '') ; ?></sTRONG> 
  ...
  <tr>
     ...
     $(document).ready(function(){
		 ...
		 if($(this).get(0).name=='opcion_pago'){
		  <?php $pagof = $toursbooking['tqp']*$toursbooking['totalpax'];  
			 ...
			 if($(this).val()=='5'){
				  <?php 
						if($toursbooking['totaltotal']>$disponible && $acountAgen->opcion5 != 1){
							?>
    ...
    function actualizarAmount(amount){
	   ...
	   $('#total_amount').html('$ '+valor);
	    var num = document.getElementsByName('opcion_pago').length
		 ...
		 if(typePago==3){
			 $('#balance').html('$ '+formato('<?php echo ($toursbooking['4fee']+$pagob);?>',2));
			  $('#fee_collect').html('$ '+formato('<?php echo ($toursbooking['4fee']);?>',2));
		 }else{
    ...
    $("#btn-continue").click(function(){
		if(!$('#ter2').is(":checked") || !$('#ter1').is(":checked") ){     
			alert(' PLEASE READ THE TERMS AND BOOK ING COND IT IONS');
			return false;
		}if (validateForm()){
	  ...
	  $('#form1').submit();
							...
							if (validateForm()){
                                   $('#form1').attr("action","<?php echo $data['rootUrl']; ?>tours/quotationOnedayTour");  
							   $('#form1').submit();
                                                        }else{
							   return false;
							}
