...
class FacturacionController extends I18nController {
    ...
    public function get_reserves() {
        ...
        $sql = "select * from ((
                ...
                union
                (select 'RESERVE' as 'type_service', r.comments as comments, r.id, r.totaltotal as 'total_amount', r.estado, r.codconf as codigo, IFNULL(a.company_name,IF(r.canal = 'WEBSALE','Supertours WEBSALE','Supertours')) as 'agencia', r.fecha_ini, r.firsname, r.lasname from reservas as r left join clientes as c on c.id = r.id_clientes left join agencia as a on a.id = r.agency where (r.estado = 'CONFIRMED'  OR r.estado = 'NOT SHOW W/ CHARGE' OR r.estado_one = '3' OR r.estado_one = '5' OR r.estado_round = '3' OR r.estado_round = '5' ) and (r.fecha_salida_ns between '$f_ini' and '$f_fin') and id_tours = -1  ) ORDER BY agencia desc)) as U where $filtro";
    ...
    public function process_invoice() {
        ...
        try {
            ...
            //damos formato a la variable para ser enviada al campo creation_date de la tabla factura   
                $nueva_fecha = date('Y-m-d', strtotime($inv_fecha ));
...
//        exit;
    ...
    public function get_invoice() {
        try {
            ...
            //averiguar el id de la compania de la factura para saber si colocar la informacion de la agencia o
    ...
    public function genpdf() {
        ...
        $sql = "SELECT t1.*, t2.starting_date AS fecha   FROM facturaservicio t1
                     LEFT JOIN tours_oneday t2 ON (t1.id_servicio = t2.id)
                     WHERE t1.id_factura = ?
                       AND t1.TIPO_SERVICIO = 'ONE'
                     UNION
                     SELECT t1.*,  t2.starting_date AS fecha FROM facturaservicio t1
                     LEFT JOIN tours t2 ON (t1.id_servicio = t2.id)
                     WHERE t1.id_factura = ?
                       AND t1.TIPO_SERVICIO = 'MULTI'
                     UNION
                     SELECT t1.*,  t2.fecha_salida_ns AS fecha FROM facturaservicio t1
        ...
        foreach ($rs as $service) {
            ...
            if ($service['tipo_servicio'] == "ONE") {
                ...
                //$sdesc['fecsal'] = $res->fecha_salida_ns;
...
//                $sql4 = "SELECT fecha_salida_ns  FROM reservas WHERE id_reserva = ?";
//                $q4 = Doo::db()->query($sql4);
//                $rs4 = $q4->fetch();
