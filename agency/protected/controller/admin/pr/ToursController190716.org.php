...
//            $select = 4;
    ...
    public function acomodacion() {
        ...
        echo '&nbsp;&nbsp;&nbsp;
		<ul style="display:inline-table; border: 1px solid #000;"  > ';
        for ($i = 1; $i <= $rooms; $i++) {
...
//                       print_r($fecha_fin);
                ...
                if (!empty($consulta) && $consulta ['id_parque'] == 0) {
                    ...
                    $sql3 = 'SELECT  adults,child,id_parque,cantidad,fecha_ini
								FROM admin_parques_tarifa
								WHERE type_rate = 1 AND id_agency =-1 AND  id_parque = ? AND cantidad >= 1 AND fecha_ini <= ?  AND fecha_fin >= ?';
                    $sql03 = 'SELECT  adults,child,id_parque,cantidad
								FROM admin_parques_tarifa
								WHERE type_rate = 2 AND id_agency = ? AND  id_parque = ? AND cantidad = 1  AND fecha_ini <= ?  AND fecha_ini >= ?';
                    if ($dat->id != -1) {
                        $rs3 = Doo::db()->query($sql03, array(
                            $dat->id,
                            trim($park['id_park']), $fecha_ini, $fecha_fin
                        ));
                        $consulta3 = $rs3->fetch();
                        if (empty($consulta3)) {
                            $rs3 = Doo::db()->query($sql3, array(
                                trim($park['id_park']), $fecha_ini, $fecha_fin
                            ));
                            $consulta3 = $rs3->fetch();
                        }
                    ...
                    //////////////////////////////////////////////////////////////////////////////////////////////////////
                    if (!empty($consulta3)) {
                        if ($consulta3['cantidad'] == 1 && $consulta3['id_parque'] != 0) {
                           
                                                      //retorno de consulta
                            
                            $priceChild = $consulta3['child'];
                            $priceAdult = $consulta3['adults'];
