...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    public function ticketPark($dat, $adult, $child, $fecha_retorno) {
        ...
        foreach ($atraccion as $id_grupo => $grupo) {
            ...
            foreach ($grupo as $id_park => $park) {
                if ($dat->id != -1) {
                    $rs = Doo::db()->query($sql0, array($dat->id, trim($id_grupo), trim($cantidad), $fecha_fin, $fecha_fin));
                    $consulta = $rs->fetch();

                    if (empty($consulta) || $consulta ['id_parque'] != 0) {
                        $rs = Doo::db()->query($sql, array(trim($id_grupo), trim($cantidad), $fecha_fin, $fecha_fin));
                        $consulta = $rs->fetch();
                    }
                } else {
                    $rs = Doo::db()->query($sql, array(trim($id_grupo), trim($cantidad), $fecha_fin, $fecha_fin));
                ...
                if (!empty($consulta) && $consulta ['id_parque'] == 0) {
                    ...
                    if ($dat->id != -1) {
                        ...
                        if (empty($consulta2)) {
                            $rs = Doo::db()->query($sql2, array(trim($park['id_park']), $fecha_fin, $fecha_fin));
                            $consulta2 = $rs->fetch();
                        }
                    } else {
                        $rs = Doo::db()->query($sql2, array(trim($park['id_park']), $fecha_fin, $fecha_fin));
