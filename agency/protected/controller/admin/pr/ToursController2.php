        ...
        foreach ($atraccion as $id_group => $grupo) {
            foreach ($grupo as $id_park => $park) {
                ...
                $tabla .= '<tr class="row0">
								...
								<td title="Chil: ' . $ticket['child'] . ', Adult: ' . $ticket['adult'] . '"> $ ' . $ticket['adult'] . " / " . $ticket['child'].'</td>
								<td title="Chil: ' . $transpor['child'] . ', Adult: ' . $transpor['adult'] . '"> $ ' . $transpor['adult'] . " / " . $transpor['child'].'</td>                                                                   
            ...
            foreach ($grupo as $id_park => $park) {
                ...
                if (!empty($consulta) && $consulta ['id_parque'] == 0) {
                    ...
                    if ($dat->id != -1) {
                        ...
                        $rs = Doo::db()->query($sql2, array(trim($park['id_park']),$fecha_fin));
                        $consulta = $rs->fetch();
                        //print_r($consulta);
                    }
                    ...
                    if (!empty($consulta)) {
                        if ($consulta['cantidad'] == 1 && $consulta['id_parque'] != 0) {
                           ...
                           // $park['ticket'] = array("child" => $consulta['child'], "adult" => $consulta['adults'], "cantidad" => $cantidad_consulta, "v_p_child" => $priceChild, "v_p_adult" => $priceAdult);
                           // $park['ticket'] = array("child" => $consulta['child'], "adult" => $consulta['adults'], "precio_varios" => 1, "cantidad" => $cantidad_consulta, "v_p_child" => $priceChild, "v_p_adult" => $priceAdult);
//                            if ($adultos == 0) {
//                                $parkFree ++;
//                            }
        ...
        foreach ($atraccion as $id_grupo => $grupo) {
            foreach ($grupo as $id_park => $park) {
                ...
                if ($grupo_array[$id_grupo] > 0) {
                    ...
                    if ($dat->id != -1) {
                        $rs = Doo::db()->query($sql0, array($dat->id, trim($id_grupo), trim($cantidad),$fecha_fin));
                        $consulta = $rs->fetch();

                        if (empty($consulta) || $consulta ['id_parque'] != 0) {
                            $rs = Doo::db()->query($sql, array(trim($id_grupo), trim($cantidad),$fecha_fin));
                            $consulta = $rs->fetch();
                        }
                    } else {
                        $rs = Doo::db()->query($sql, array(trim($id_grupo),trim($cantidad),$fecha_fin));
