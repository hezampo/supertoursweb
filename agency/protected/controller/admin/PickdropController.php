...
class PickdropController extends I18nController{
    ...
    public function index(){
        ...
        $rs = Doo::db()->query("SELECT t1.id, t1.place, t1.address, t1.type_web, t1.active_web, t1.valid, t1.valid, t1.posicion, t2.nombre
                                FROM pickup_dropoff t1
                                LEFT JOIN areas t2 ON (t1.id_area = t2.id)                                
                                where t1.$filtro like ?  order by t1.posicion asc limit $pager->limit ", 
                                array($texto.'%'));
...
public function add(){
    ...
    public function edit() {
      ...
      $rs =  Doo::db()->query("SELECT t1.id, t1.place, t1.address, t2.nombre, t1.posicion, t1.type_web, t1.active_web 
                               FROM pickup_dropoff t1	
                               LEFT JOIN areas t2 ON (t1.id_area = t2.id)
