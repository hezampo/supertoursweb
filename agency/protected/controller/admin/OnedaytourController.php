...
class OnedaytourController extends I18nController {
    ...
    public function index() {
        ...
        if (isset($this->params['pindex']))
            ...
            $pager->paginate(1);
        $sql = "SELECT id, priceadult_WDW, pricechild_WDW, priceadult_WATERP, pricechild_WATERP, priceadult_KENNEDYSPC, pricechild_KENNEDYSPC, start_date, end_date, type_rate, id_agency, annio
                              " . (($type_rate == 2) ? ",company_name" : "") . "  
                              FROM onetour WHERE " . $where . "  order by start_date DESC limit $pager->limit";
...
//            $tarifaonetour = new Onetour($tarifaonetour[0]);
            ...
            $tarifaonetour->start_date = $start_date;
            $tarifaonetour->end_date = $end_date;            
            $tarifaonetour->annio = $_POST['annio'] . '-01-01 00:00:00';
            
            $tarifaonetour->company_name = $company_name;     
...
//        } else {
            ...
            $id_agency = $_POST['id_agency'];  
            $id_onetour = $_POST['id_onetour'];
            //$annio = $_POST['annio'] . '-01-01 00:00:00';
            ...
            $priceadult_WDW = $_POST['priceadult_WDW'];
            $pricechild_WDW = $_POST['pricechild_WDW'];
            $priceadult_WATERP = $_POST['priceadult_WATERP'];
            $pricechild_WATERP = $_POST['pricechild_WATERP'];
            $priceadult_KENNEDYSPC = $_POST['priceadult_KENNEDYSPC'];
            $pricechild_KENNEDYSPC = $_POST['pricechild_KENNEDYSPC'];
            
            if (Doo::db()->query("UPDATE onetour SET type_rate='$type_rate', id_agency='$id_agency', company_name='$company_name', priceadult='$priceadult', pricechild='$pricechild', priceadult_WDW='$priceadult_WDW', pricechild_WDW='$pricechild_WDW', priceadult_WATERP='$priceadult_WATERP', pricechild_WATERP='$pricechild_WATERP', priceadult_KENNEDYSPC='$priceadult_KENNEDYSPC',pricechild_KENNEDYSPC='$pricechild_KENNEDYSPC',start_date='$start_date',end_date='$end_date' WHERE id='$id_onetour'")){
...
//        }
        ...
        $sql = "SELECT p.nombre, t.adults, t.child, t.cantidad, p.id_grupo AS gid, g.nombre AS gnombre
            ...
            WHERE p.id_grupo = t.id_grupo  AND t.cantidad =1 AND t.id_parque = ? AND p.id = ?
            AND type_rate =1 AND t.id_agency = ? AND g.id = p.id_grupo AND t.fecha_ini <= ? AND t.fecha_fin >= ?
            ORDER BY t.cantidad DESC LIMIT 1";

        $rs = Doo::db()->query($sql, array($id_park, $id_park, $id_agency, $fecha3, $fecha6));
        ...
        if ($parks_rates_rows == 0) {
            $sql = "SELECT p.nombre, t.adults, t.child, t.cantidad, p.id_grupo AS gid, g.nombre AS gnombre
            FROM parques AS p, admin_parques_tarifa AS t, grupo_parques AS g
            WHERE p.id_grupo = t.id_grupo AND t.cantidad =1 AND t.id_parque = ? AND p.id = ? AND type_rate =1
            AND g.id = p.id_grupo AND t.fecha_ini <= ? AND t.fecha_fin >= ? ORDER BY t.cantidad DESC LIMIT 1";
            //$rs = Doo::db()->query($sql, array($id_park, $fecha));
            $rs = Doo::db()->query($sql, array($id_park, $id_park, $fecha3, $fecha6));
        }
...
//        print_r($parks_rates);
        ...
        if (count($parks_rates) > 0) {
            ...
            if (!$especial) {
                $transport1 = ($id_park == 19) ? 0 * ($childs + $adults) : 0;
            } else {
                $transport1 = 0;
            }
...
//                         $("#total_sumplemento").val(' . $suplemento_suma . ');    
    ...
    public function getCosto() {
        ...
        $total_pax = $this->params['total_pax'];
        //echo $total_pax;
        ...
        list($dia, $mes, $anyo) = explode("-", $fecha4);
        $fecha5 = $mes . "-" . $dia . "-" . $anyo;
        $fecha_sal = $anyo . "-" . $dia. "-" . $mes;
        $fecha_salida = strtotime($fecha5); 
        //echo $disponible300;
...
//        $remain_301 = $main->disponible('301', $fecha_sal);
//        $disponible301 =  $remain_301 - $total_pax;   
//        
//        $remain_300 = $main->disponible('300', $fecha_sal);
//        $disponible300 =  $remain_300 - $total_pax;   
//        
        Doo::loadModel("Agency");
...
/////////////////Aqui debemos controlar los costos actualizados para ONE DAY TOURS por GRUPOS DE PARQUES///////////////////////
        //|| $dat->type_rate == 0 
        if ($dat->type_rate == 1) {// Se busca SpecialNet , $fecha
            $sql = "SELECT onet.priceadult_WDW, onet.pricechild_WDW, onet.priceadult_WATERP, onet.pricechild_WATERP, onet.priceadult_KENNEDYSPC, onet.pricechild_KENNEDYSPC   
                    FROM  onetour onet 
                    WHERE onet.type_rate = ? AND id_agency = ?  AND start_date <= '$fecha_salida' AND end_date >= '$fecha_salida'";
            $type = 2;
            $rs = Doo::db()->query($sql, array($type, $dat->id));
            $prices = $rs->fetch();
            //print_r($prices);


            //AND annio = ?
            if (empty($prices)) {//Si no encuentra Buscamos Net., $fecha
                $sql = "SELECT onet.priceadult_WDW, onet.pricechild_WDW, onet.priceadult_WATERP, onet.pricechild_WATERP, onet.priceadult_KENNEDYSPC, onet.pricechild_KENNEDYSPC 	
                        FROM  onetour onet 
			WHERE onet.type_rate = ?  AND start_date <= '$fecha_salida' AND end_date >= '$fecha_salida'";
                $type = 1;
        ...
        } else {//Buscamos Comi  AND annio = ? , $fecha
            $sql = "SELECT onet.priceadult_WDW, onet.pricechild_WDW, onet.priceadult_WATERP, onet.pricechild_WATERP, onet.priceadult_KENNEDYSPC, onet.pricechild_KENNEDYSPC  
                    FROM  onetour onet
                    WHERE onet.type_rate = ? AND start_date <= '$fecha_salida' AND end_date >= '$fecha_salida'";
            $type = 0;
...
//            unset($_SESSION['codconf']);
        ...
        $cod_conf = $_POST['reser'];
        $totalAmount = $_POST['totalAmount'];

        $incluir_parke = $_POST['incl_ticket'];
        ...
        $total_charge = $_POST['temp'];
        
        
        //$Btnpay = $_POST['textpay'];         
        //AMOUNT TO COLLECT
        ...
        //OPCION PAGO CONDUCTOR
        
        
//        $opcion_pago_prepagado = $_POST['opcion_pago_2']; //op_pago_id2
//        $opcion_pago_conductor = $_POST['op_pago_conductor']; 
//        $opcion_pago_collectonboard = $_POST['opcion_pago']; //op_pagp_id 
        
        $opcion_pago_one = $_POST['opcion_pago_one'];
        $opcion_pago = $opcion_pago_one;

//        echo "<<".$opcion_pago_collectonboard;
//        exit();
//        echo '<<<'.$passenger_balance_due;
//        
//        echo '<<<'.$agency_balance_due;  
//               
//        echo '<<<<<<'.$totale;
//        echo '<<<<<<'.$totaleouta;
//        echo '<<<<<<'.$total_charge;
...
//
        ...
        try {
            ...
            //onet.priceadult, onet.pricechild,
            $especial = false;
            if ($dat->type_rate == 1) {// Se busca SpecialNet
                $sql = 'SELECT 
                                    onet.priceadult_WDW,
            ...
            $rs = Doo::db()->query($sql, array($id_selected_park, $id_selected_park, $id_agency, $fecha2, $fecha4));
            $parks_rates_rows = $rs->rowCount();
            if ($parks_rates_rows == 0) {
                $sql = "SELECT p.nombre, t.adults, t.child, t.cantidad, p.id_grupo AS gid, g.nombre AS gnombre
                    FROM parques AS p, admin_parques_tarifa AS t, grupo_parques AS g
                    WHERE p.id_grupo = t.id_grupo AND t.cantidad =1 AND t.id_parque = ? AND p.id = ?
                    AND type_rate =1 AND g.id = p.id_grupo  AND t.fecha_ini <= ? AND t.fecha_fin >= ?
                    ORDER BY t.cantidad DESC LIMIT 1";

                //$rs = Doo::db()->query($sql, array($id_selected_park, $fecha));
                $rs = Doo::db()->query($sql, array($id_selected_park, $id_selected_park, $fecha2, $fecha4));
            }
            $parks_rates = $rs->fetchAll();
...
//            }
            ...
            //numero de pago siendo:
            // 1. para agency credit card
            // 2. passenger credit card
            //3. credit card +4 fee 
            //4. cash 
            //5. credit voucher 
            //6. cash in terminal
            //7. complementary ademas
            // se concatenara - y
            // 1 si es paid full,
            // 2 si es paid balance o 
            // 0 si no es ninguno de las 2 opciones
...
//            exit();
            ...
            if($opcion_pago == 1){//Credit card w/f
                $opcion_pago_res = 3;  
                $reserve->tipo_pago = "PRED-PAID";
            }
            
            if($opcion_pago == 2){//Credit card no fee
                $opcion_pago_res = 8;  
                $reserve->tipo_pago = "PRED-PAID";
            }
            
            if($opcion_pago == 6){//Cash
                $opcion_pago_res = 4; 
                $reserve->tipo_pago = "PRED-PAID";
            }
            
            if($opcion_pago == 10){//Check
                $opcion_pago_res = 9;
                $reserve->tipo_pago = "PRED-PAID";
            }
            
            if($opcion_pago == 5){//Voucher
                $opcion_pago_res = 5;
                $reserve->tipo_pago == "CREDIT VOUCHER";
            }
...
//            exit();
            ...
            if (isset($opcion_saldo)) {
                $reserve->pago = $opcion_pago_res . '-' . $opcion_saldo; 
            } else {
                $reserve->pago = $opcion_pago_res . '-0';
            }
...
//                 echo 'PHONE';
            ...
            $sqlu = "SELECT nombre as OPERADOR FROM  usuarios  WHERE id = $user";

            $rsu = Doo::db()->query($sqlu);
            $usuario = $rsu->fetch();
            ...
            $tour->creation_date = date("Y-m-d H:i:s");
            list($m, $d, $y) = explode("-", $fecha_salida);
            ...
            $comments1 = strtoupper($comments);
            //$tour->comments2 = $comments;
            //$tour->comments2 = ("New Note: " . $comments1 . " " . $fecha . ";\n" . $operador1 . ".");            
            //$tour->comments2 = ("NEW NOTE: " . " \n\n"  . $comments1 . " \n\n" . $fecha . "; USER: " . $operador1 . ".");
            $tour->comments2 = ("NEW NOTE: " . " \n\n"  . $comments1 . " \n\n" . $fecha . "; USER: " . $operador1 . "." ."\n".'CONFIRMATION #:'."". $codeconfone ."");
            $tour->passenger_balance_due = $passenger_balance_due;
            $tour->paid_driver = $paid_driver;
            $tour->agency_balance_due = $agency_balance_due;
            $tour->include_park = $incl_park;


            //************Actualizacion Agosto 21 de 2018////////
            
            //list($m, $d, $y) = explode("-", $fecha_salida);
            //date1 fecha para tabla traffic_report
            $date1 = $y . '-' . $m . '-' . $d;         
            ...
            $sqlt = "SELECT MAX(id)+1 AS PROX FROM tours_oneday";
            $rst = Doo::db()->query($sqlt);
            $id_final = $rst->fetchAll();

            foreach ($id_final as $reg_final){

                $proximo_registro = $reg_final['PROX'];
            }


            Doo::db()->query("INSERT INTO traffic_report
                       (id,id_tour,type_tour,time_am,from1,to1,id_attraction_trafic,date1,id_cliente,time_pm,id_bus_am,id_bus_pm,type_traffic,driver_am,driver_pm,type_ticket,
                        hotel_name,parking,estado)
                VALUES ('','$proximo_registro','ONE','08:00:00','Supertours Terminal','Supertours Terminal','00000','$date1','$cliente->id','18:00:00','','','PARK','','','','ONE DAY TOUR','','');");
            ...
            $transport = ($id_selected_park == 19) ? 0 : 0;
...
//            $tour->total = $totale;
//            $tour->totalouta = $totaleouta;
            
            
            $tour->total = $totalAmount;//$total_total_new;
            $tour->totalouta = $totalAmount;//$total_total_new_outa;            
            $tour->pred_paid_amount = $pred_paid_amount;
            $tour->total_charge = $total_charge;
            $tour->total_paid = $total_paid;


            //echo $total;
//            print($total_total_new);
//            print($total_total_new_outa);
//            exit();
...
//            exit;
            ...
            $tour->code_conf = $reserve->codconf;//$cod_conf;
            ...
            if($opcion_pago == 1){//Credit card w/f
                $op_pago_res = 3;  
                $opcion_pago_conductor = 3;
            }
            if($opcion_pago == 2){//Credit card no fee
                $op_pago_res = 8;  
                $opcion_pago_conductor = 8;
            }
            if($opcion_pago == 6){//Cash
                $op_pago_res = 4;  
                $opcion_pago_conductor = 4;
            }
            if($opcion_pago == 10){//Check
                $op_pago_res = 9;    
                $opcion_pago_conductor = 9;
            }
            if($opcion_pago == 5){//Voucher
                $op_pago_res = 5;  
                $opcion_pago_conductor = 5;
            }
            ...
            $tour->op_pago = $op_pago_res;
            $tour->op_pago_conductor = $opcion_pago_conductor;
...
//                }
            ...
            if ($pay_amount != "" && $pay_amount > 0) {
                ...
                //$this->registrar_pago($tour, NULL, $login, $pay_amount, $prefix, $tipo_pag);
                    ...
                    if($no_prep == 1){
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagadopre1, $tipo_pagopre1, $pagopre1);                   
                        
                    }else if($no_prep == 2){
                        
                        $this->registrar_pago($tour, NULL, $login, $pagadopre1, $tipo_pagopre1, $pagopre1);
                        
                        $this->registrar_pago($tour, NULL, $login, $pagadopre2, $tipo_pagopre2, $pagopre2); 
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagadopre3, $tipo_pagopre3, $pagopre3);
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagadopre4, $tipo_pagopre4, $pagopre4);                       
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagadopre5, $tipo_pagopre5, $pagopre5);    
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagadopre6, $tipo_pagopre6, $pagopre6); 
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagadopre7, $tipo_pagopre7, $pagopre7); 
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagadopre8, $tipo_pagopre8, $pagopre8); 
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagadopre9, $tipo_pagopre9, $pagopre9); 
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagadopre10, $tipo_pagopre10, $pagopre10); 
                    }            
            ...
            if ($paid_driver != "" && $paid_driver > 0) {
                ...
                $rastrostring .= "Paid = " . $paid_driver . '&';

                if($no_pago == 1){
                        
                        $this->registrar_pago($tour, NULL, $login, $pagado1, $tipo_pago1, $pago1);
                        
                    }else if($no_pago == 2){
                        
                        $this->registrar_pago($tour, NULL, $login, $pagado1, $tipo_pago1, $pago1);
                        
                        $this->registrar_pago($tour, NULL, $login, $pagado2, $tipo_pago2, $pago2);
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagado3, $tipo_pago3, $pago3);
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagado4, $tipo_pago4, $pago4);
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagado5, $tipo_pago5, $pago5);
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagado6, $tipo_pago6, $pago6);
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagado7, $tipo_pago7, $pago7);
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagado8, $tipo_pago8, $pago8);
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagado9, $tipo_pago9, $pago9);
                        ...
                        $this->registrar_pago($tour, NULL, $login, $pagado10, $tipo_pago10, $pago10);
...
//                return Doo::conf()->APP_URL . 'admin/onedaytour/edit/' . $tour->id . "?menssage=Guardado Correctamente";
            ...
            echo "<script> window.open('../onedaytour/edit/$tour->id','ONEDAYSAVE','')</script>";
        } catch (Exception $e) {
            Doo::db()->rollBack();
            print_r($e);
            exit;
//            return Doo::conf()->APP_URL . 'admin/onedaytour/add/?error=error al guardar';
            ...
            return Doo::conf()->APP_URL . "admin/onedaytour/add";
//            echo $e;
        }

        echo "<script> window.close('../onedaytour/save','','')</script>";
    }
...
//        $tour->t_price_adult = $_POST["priceadults"];
//        $tour->t_price_child = $_POST["pricechilds"];
//        $precio_adulto = $_POST["priceadults"];
//        $precio_nino = $_POST["pricechilds"];   
//        $Btnpay = $_POST['textpay'];   
        ...
        $deptime1 = $_POST['deptime1'];
        $deptime2 = $_POST['deptime2'];
        
        $arrtime1 = $_POST['arrtime1'];
        $arrtime2 = $_POST['arrtime2'];
...
//        echo $codigo_confirm;
        ...
        $opcpago = $_POST['opcionpago'];
        $estado_transfer_out = $_POST['estado_trf_out'];
        $fecha_salida = $_POST['fecha_salida'];    
        ...
        $fecha_retorno1 = $_POST['fecha_retorno'];        
        $fec_retor = $_POST['fec_retor'];
        $trip_no = '300';
        
        
        $sqlres = "SELECT id as idreserva, id_bus as idbus FROM reservas WHERE  codconf = '$codigo_confirm' AND type_tour = 'ONE' AND trip_no2 = '300' AND fecha_salida = '$fecha_sal'";
        $rsres = Doo::db()->query($sqlres);
        $id_r = $rsres->fetchAll();
       
        
        foreach ($id_r as $reser){
            
            $id_reserva = $reser['idreserva'];
            $id_bus = $reser['idbus'];
        }         
         ...
         if($fec_retor == 'N/S'){
             ...
             $fecha_retorno1 = 'N/A';
             Doo::db()->query("UPDATE passengers_bus SET  id_bus = '-1'  WHERE fec_ini ='$fecha_sal'  AND trip = '$trip_no' AND id_reservas = '$id_reserva'");                           
             ...
             $fecha_retorno1 = $fec_retor;
             Doo::db()->query("UPDATE passengers_bus SET  id_bus = '$id_bus'  WHERE fec_ini ='$fecha_sal'  AND trip = '$trip_no' AND id_reservas = '$id_reserva'");
...
//      opcion pago a conductor
        $opd = $_POST['opcion_pago_driver'];      
        
//      opcion pago prepagado
        $opp = $_POST['opcion_pago_3'];
        
        $grupo_parque = $_POST['group_park2'];
        $total_charge = $_POST['tot_charge'];
        $totale = $_POST['saldoactual'];
        $totaleouta = $_POST['totalAmount'];
        $other_amount = $_POST['otheramountp'];        
        $passenger_balance_due = $_POST['balance_due'];
        $paid_driver = $_POST['paid_driverp'];
        $agency_balance_due = $_POST['agency_balance_due'];
        $total_paid = $_POST['tot_amount_paidp'];
...
//            $cliente = Doo::db()->getOne($cliente);
            ...
            //onet.priceadult, onet.pricechild,
            if ($dat->type_rate == 1) {// Se busca SpecialNet
                $sql = 'SELECT      onet.priceadult_WDW, onet.pricechild_WDW,
                                    onet.priceadult_WATERP,onet.pricechild_WATERP,
                                    onet.priceadult_KENNEDYSPC,onet.pricechild_KENNEDYSPC,
                                    onet.suplemag_adult,
                                    onet.suplemag_child,
                                    onet.suplepcot_adult,
                                    onet.suplepcot_child,
                ...
                if (empty($prices)) {
                    $sql = 'SELECT onet.priceadult_WDW, onet.pricechild_WDW,onet.pricechild_WDW, onet.priceadult_WATERP,onet.pricechild_WATERP, onet.priceadult_KENNEDYSPC,onet.pricechild_KENNEDYSPC
                        FROM  onetour onet WHERE onet.type_rate = ? AND annio = ?';
...
//                exit;
            ...
            $fecha_salida2 = $y . '-' . $m . '-' . $d;
            $fecha_retorno2 = $y2 . '-' . $m2 . '-' . $d2;
            
            $reserve->fecha_salida_ns = $fecha_salida2;
            $reserve->fecha_retorno = $fecha_retorno1;  
            $reserve->fecha_retorno_ns = $fecha_retorno2; 
            $reserve->pax3 = 0;
            ...
            $reserve->tot = $to;
            $reserve->fromt2 = $reserve->tot;
            $reserve->estado_round = $estado_trf_out;
...
//            $tour->comments = $notes;
            ...
            $fechaone = date("[Y-m-d  H:i:s]", $time);
            
            $user = $_SESSION['login']->id;

            $sqlop = "SELECT nombre as OPERADOR FROM  usuarios  WHERE id = $user";
            ...
            $sqlcom = "SELECT comments2 FROM  tours_oneday  WHERE id_reserva = $tour->id_reserva";

            $rscom = Doo::db()->query($sqlcom);
            $commen2 = $rscom->fetch();
//            print_r($commen2);
//            exit;            
            $notes2 = $commen2['comments2'];
            $notes3 = strtoupper($notes2);
//            $salto = '<br>';

            
            $tour->comments2 = ("NEW NOTE: " ."\n\n". $comments1 . " \n\n" . $fechaone . "; USER: " . $operador1 . "." ."\n".'CONFIRMATION #:'."". $cod_conf ."". "\n\n" . $notes3);
            ...
            /** Discriminacion de precios */
            $tour->t_price_adult = $_POST["priceadults"];
            //echo $tour->t_price_adult;
            $tour->t_price_child = $_POST["pricechilds"];
            // echo $tour->t_price_child;
            //prueba de resulatdos para oneday     
            //print_r($tour);
            //exit;
...
//            exit;
            //$pay_amount= $pred_paid_amount;

            $tour->total = $totalAmount;//$total_total_new;

            $tour->totalouta = $totalAmount;//$total_total_new_outa;

            $tour->group_park = $grupo_parque;

            $tour->total_charge = $total_charge;
            
            $tour->op_pago = $opcion_pago_conductor;//$opcpago;
...
//          $tour->comments = $comments;
            ...
            //if (!$tour->include_park && isset($incl_park)) {
                //$cambio_tickets = true;
            //}
...
//            }
            ...
            //$tour->otheramount = $otheramount + $tres_porciento;
            $tour->otheramount = $other_amount;
            $tour->otheramount_sin_tax = $other_amount;
            ...
            //Actualizacion Agosto 23 de 2018
            Doo::loadModel('Traffic_report');
            $traf_report = new Traffic_report();

            Doo::db()->query("DELETE FROM  traffic_report WHERE id_tour = '$tour->id' AND type_tour ='ONE'");
            ...
            //insertamos el nuevo parque por si cambiaron algo del anterior
            $parks = new Attraction_Trafic();
            $parks->type_tour = "ONE";
            $parks->id_tours = $tour->id;
            $parks->id_park = $id_selected_park;
            ...
            Doo::db()->query("INSERT INTO traffic_report
                       (id,id_tour,type_tour,time_am,from1,to1,id_attraction_trafic,date1,id_cliente,time_pm,id_bus_am,id_bus_pm,type_traffic,driver_am,driver_pm,type_ticket,
                        hotel_name,parking,estado)
                VALUES ('','$tour->id','ONE','08:00:00','Supertours Terminal','Supertours Terminal','00000','$date1','$cliente->id','18:00:00','','','PARK','','','','ONE DAY TOUR','','');");
...
//                $prefix = "COLLECT ON BOARD";
                ...
                //$this->registrar_pago($tour, NULL, $login, $pago_conductor, $prefix, $tipo_pag);
...
//                $prefix2 = "PRE-PAID";
                ...
                //$this->registrar_pago($tour, NULL, $login, $pago_prepago, $prefix2, $tipo_pre);
...
//            exit;
	...
	p6.nombre AS parque,
        p7.passenger_balance_due AS balance_pasajero,
        p7.agency_balance_due AS balance_agencia,
        p7.total_paid AS total_pagado,
        p7.t_price_adult AS precioAdultoParque,
        p7.entradas_price_adult AS entradaAdult,
        p7.extra_charge as extracargo,
        p7.entradas_price_child AS entradaChild
	...
	LEFT JOIN parques p6 ON (p5.id_park = p6.id)
        LEFT JOIN tours_oneday p7 ON (r1.id_clientes = p7.id_client) 
	WHERE r1.id_tours  = '$prueba'";
...
//formateamos la variable para entregarla con dos cifras decimales
        ...
        //$apellido = strtoupper($key['apellido']);
        ...
        $extracharge = $key['extracargo'];
        $extracargo = number_format($extracharge, 2, '.', '');
        ///echo $admision;
        ...
        //echo $precioA, ' esto por ', $adult, '<br/>';
//        $rprecioA = [($precioA * $adult)+$nxc];
//        $rprecioA1 = number_format($rprecioA, 2, '.', '');
//        echo $rprecioA1, '<br/>'; 
//        $precioC = number_format($precioChild, 2, '.', ''); 
//        $rprecioC = ($precioC * $child);
//        $rprecioC1 = number_format($rprecioC, 2, '.', '');        
//        echo $precioC, ' esto por ', $child, '<br/>';
        //echo $rprecioC1;
//        $precioparques = ($rprecioA1 + $rprecioC1);
//        $precioparques1 = number_format($precioparques, 2, '.' , '');
//        echo '<br/>', $precioparques1;
        ...
        //$precioparques1 = number_format($precioparques, 2, '.' , '');
        //echo '<br/>', $precioparques1; 
        //precio de los bastarditos
        $precioC = number_format($precioChild, 2, '.', '');
        $rprecioC = ($precioC * $child) + $nxc;
        $rprecioC1 = number_format($rprecioC, 2, '.', '');
        ...
        //echo $precioC, ' esto por ', $child, '<br/>';
//        echo 'precio por unidad mas valor de la transportacion bastarditos ',$rprecioC1, '<br/>';  
        //echo $rpromedioC1;
//        
        ...
        $total_paid1 = $key['total_pagado'];
        $total_paid = number_format($total_paid1, 2, '.', '');
...
//Primera letra en Mayuscula
        ...
        $sql6 = "SELECT SUM(pagado) AS CARGO2 FROM tours_pago 
                    
                    WHERE id_tours = '$prueba' AND tipo ='ONE' AND (pago = 'CREDIT CARD WITH FEE' OR tipo_pago = 'CREDIT CARD WITH FEE')";


        $rs6 = Doo::db()->query($sql6);
        $cargoscc = $rs6->fetchAll();

        foreach ($cargoscc as $clave5 => $key5) {
            
        }

        $total_charge2 = $key5['CARGO2'];
        $totalent = $total_charge2/1.04;
        $totdec = $total_charge2 - $totalent;       
        $total_chargef = number_format($totdec, 2, '.', '');
        

        $sql5 = "SELECT op_pago FROM tours_oneday 
...
//        $service_fee1 = ($rprecioA1 + $rprecioC1 + $extracargo) * 0.04;
        $service_fee = $total_chargef;

        $totalApagar = ($rprecioA1 + $rprecioC1 + $extracargo) + $service_fee;
        $totalApagar1 = number_format($totalApagar, 2, '.', '');
...
//
//            $service_fee = '0.00';
//            $totalApagar = ($rprecioA1) + ($rprecioC1) + ($extracargo);
//            $totalApagar1 = number_format($totalApagar, 2, '.', '');
//        }
//
...
//            $service_fee = number_format($service_fee1, 2, '.', '');
//
//            $totalApagar = ($rprecioA1 + $rprecioC1) + $service_fee;
//            $totalApagar1 = number_format($totalApagar, 2, '.', '');
//        }
//
...
//        }
        ...
        $codigoHTML.='
            ...
            <div id="contenedor">
                ...
                <div id="sp-div-left" style="margin-left: 390px;">
                    <p style="font-size: 25px; margin-top: -8%;">
        ...
        <div id="contenido">
            <p style="font-size: 27px;  margin-left: 45%; margin-top: -2.5%;">
                E-Ticket
            </p>
            <p style="font-size: 27px;">
                <i style="font-size: 27px; line-height: 1em;">
                    <u>
            ...
            <table border="0" width ="550" bgcolor= "#D9E5F5" cellpadding="1" cellspacing="1" id="tabla" style="border: 1px solid #000;">
                <thead>
                    ...
                    <tr>                         
                        ...
                        <td align="center" colspan="5" style="font-size: 21px; width: 15%;"> <i><b>One Day Tours</b></i> </td>
                    </tr>
                    <tr>
            ...
            <table border="0" width ="550" bgcolor= "#D9E5F5" cellpadding="1" cellspacing="1" id="tabla" style="border: 1px solid #000;">
                <thead>
                    ...
                    <tr>                         
                        <td align="center" style="font-size: 21px; width: 25%;"> <u><b><u><i>Theme Park Selected</i></u></b></u></td>
            ...
            <div id="caja-contenido" style="margin-left: 50.5%; margin-top: -41%;">
                <table border="0" width ="275"  bgcolor= "#D9E5F5" cellpadding="1" cellspacing="1" id="tabla" style=";border: 1px solid #000;">
                ...
                <tbody>
                    <tr>
                        <td align="left" style="width: 44.5%;">Package Price Adult:</td>
                        <td align="right">$&nbsp;' . $rpomedioA1 . '</td>
                    ...
                    <tr>
                        <td align="left" style="width: 44%;">Package Price Child:</td>
                        <td align="right">$&nbsp;' . $rpromedioC1 . '</td>
                        <td align="center">X</td>
                    ...
                    <!--<tr>
                        ...
                        <td align="right"> </td>
                        <td align="right"> </td>
                        <td align="right">$&nbsp;' . $total_chargef . '</td>                        
                    </tr>-->
            ...
            <img src="global/img/codigo.png" alt="" style="width: 19%; margin-left: 83%; margin-top: 0%;" />
    ...
    //RESUMEN PARA AGENCIA   
        ...
        $pdf = new DooPDF('Summary One Day Tours'.' - '.$codconf, $codigoHTML, false, 'letter', 'letter');
        $pdf->doPDF();
        $archivo = $pdf;
...
//$archivo="resumen.pdf";
        ...
        $mailer = new AttachMailer($from, $to, "Summary One Day Tours", "Se adjunta Summary One Day Tours");
...
//                        //1. enviar el correo.
    ...
    public function eliminar_pagos_cob(){
        
        Doo::db()->loadModel("Tours_Pago");  
        //Doo::loadModel('Tours_Pago');
        Doo::db()->loadModel('Tour_oneday');
        Doo::db()->loadModel("Toursoneday_Rastro");
        
        $id_oneday = $this->params['id_oneday'];        
        $id_radio = $this->params['id_radio'];
        $saldoactual_copy = $this->params['saldoactual_copy'];
        $paid_driver_copy = $this->params['paid_driver_copy'];
        $balance_due_copy = $this->params['balance_due_copy'];
        $totalAmount_copy = $this->params['totalAmount_copy'];
        $pay_amount_copy = $this->params['pay_amount_copy'];
        $tot_amount_paid_copy = $this->params['tot_amount_paid_copy'];
        $agency_balance_due_copy = $this->params['agency_balance_due_copy'];
        $otheramount_copy = $this->params['otheramount_copy'];
        $temp_copy = $this->params['temp_copy'];        
        $usuario1 = $this->params['usuario'];
        $usuario  = str_replace("%20"," ",$usuario1);        
        $fecha_actual1 = $this->params['fecha_actual']; 
        $fecha_actual  = str_replace("%20"," ",$fecha_actual1);        
        $valor_pagado1 = $this->params['valor_pagado'];        
        $valor_pagado  = str_replace("%20"," ",$valor_pagado1);       
        $tipo_pago1 = $this->params['tipo_pago'];   
        $tipo_pago = str_replace("%20"," ",$tipo_pago1);        
        $metodo_pago1 = $this->params['metodo_pago'];   
        $metodo_pago = str_replace("%20"," ",$metodo_pago1);        
        $detalles = "Method: ".$metodo_pago."\n"."Type Pay: ".$tipo_pago."\n"."Paid: ".$valor_pagado."\n"."User: ".$usuario;
              
                 
        Doo::db()->query("DELETE FROM tours_pago WHERE id = '$id_radio' AND id_tours ='$id_oneday' AND tipo ='ONE';");
        
        Doo::db()->query("UPDATE tours_oneday SET  total = '$totalAmount_copy', totalouta = '$totalAmount_copy', paid_driver = '$paid_driver_copy', passenger_balance_due = '$balance_due_copy', pred_paid_amount = '$pay_amount_copy', total_paid = '$tot_amount_paid_copy', agency_balance_due = '$agency_balance_due_copy', total_charge = '$temp_copy', otheramount = '$otheramount_copy', otheramount_sin_tax = '$otheramount_copy'   WHERE id = '$id_oneday'");        
        
        Doo::db()->query("INSERT INTO toursone_rastro
                           (id,id_tours,tipo_cambio,detalles,fecha,usuario,tipo_usuario)
                            VALUES ('','$id_oneday','UPDATE','$detalles','$fecha_actual','39','Removed');"); 
